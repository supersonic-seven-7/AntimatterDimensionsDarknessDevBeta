...
function applyNDPowers(mult, tier) {
  ...
  if ((InfinityChallenge(5).isRunning || InfinityChallenge(11).isRunning) && player.postC4Tier !== tier) {
...
function onBuyDimension(tier) {
  ...
  if (NormalChallenge(4).isRunning || InfinityChallenge(1).isRunning || InfinityChallenge(2).isRunning) {
...
class AntimatterDimensionState extends DimensionState {
  ...
  challengeCostBump() {
    if (InfinityChallenge(6).isRunning || InfinityChallenge(11).isRunning) this.multiplyIC5Costs();
