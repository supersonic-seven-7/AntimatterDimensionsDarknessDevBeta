...
export const normalTimeStudies = [
  ...
  },
  {
    ...
    requirement: [22],
    reqType: TS_REQUIREMENT_TYPE.AT_LEAST_ONE,
    description: "You keep half of your Replicanti Boosters on Infinity"
  },
  {
    id: 41,
    cost: 16,
    requirement: [31],
    reqType: TS_REQUIREMENT_TYPE.AT_LEAST_ONE,
    description: () => `Replicanti Boosters give a ${formatX(DC.D2, 1, 1)} multiplier to Infinity Points gained`,
    effect: () => DC.D2.pow(Replicanti.galaxies.total + player.galaxies + player.dilation.totalTachyonGalaxies),
    formatEffect: value => formatX(value, 2, 1)
  },
  {
    id: 42,
  ...
  },
  {
    ...
    description: () => (Achievement(103).canBeApplied
      ? `Make the Infinity Point formula better based on Dimboosts log(x)/${formatFloat(307.8, 1)} ➜ log(x)/${formatFloat(307.8 / (1+(Math.pow(DimBoost.totalBoosts, 0.1)/25)),2)}`
      : `Make the Infinity Point formula better based on Dimboosts log(x)/${formatInt(308)} ➜ log(x)/${formatFloat(308 / (1+(Math.pow(DimBoost.totalBoosts, 0.1)/25)),2)}`),
    effect: 307.8 / (1+(Math.pow(DimBoost.totalBoosts, 0.1)/25))
  },
  {
    id: 121,
    cost: 15,
