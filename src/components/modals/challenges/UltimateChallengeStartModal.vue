...
export default {
  name: "UltimateChallengeStartModal",
  components: {
    ModalWrapperChoice
  },
  props: {
  ...
  computed: {
    challenge() {
      return UltimateChallenge(this.id);
    },
    ...
    message() {
      return `You will Big Crunch (if possible) and start a new Infinity within the Challenge with all the
        ...
        ${format(UltimateChallenge(this.id).goal)} Antimatter.
        You do not start with any Dimension Boosts or Galaxies, regardless of upgrades.`;
    },
    entranceLabel() {
      return `You are about to enter THE ULTIMATE CHALLENGE ${this.id}`;
    },
    reward() {
    ...
    condition() {
      ...
      return `Inside this ULTIMATE CHALLENGE, ${conditionOfChallenge}`;
    }
  },
  created() {
