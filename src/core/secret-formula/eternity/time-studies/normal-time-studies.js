...
export const normalTimeStudies = [
  ...
  },
  {
    ...
    description: () => (Achievement(103).canBeApplied
      ? `Make the Infinity Point formula better based on Dimboosts log(x)/${formatFloat(307.8, 1)} ➜ log(x)/${format(307.8 / (1+(Math.pow(DimBoost.totalBoosts, 0.1)/25)),2)}`
      : `Make the Infinity Point formula better based on Dimboosts log(x)/${formatInt(308)} ➜ log(x)/${format(308 / (1+(Math.pow(DimBoost.totalBoosts, 0.1)/25)),2)}`),
